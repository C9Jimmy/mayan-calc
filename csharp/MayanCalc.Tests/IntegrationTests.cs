...
{
    [Theory]
    [InlineData(2012, 12, 21, 4, "Ajaw", 3, "K'ank'in", 13, 0, 0, 0, 0, 1)]
    [InlineData(2000,  1,  1, 11, "Ik'", 10, "K'ank'in", 12, 19, 6, 15, 2, 6)]
    [InlineData(1988, 12,  7, 12, "Ajaw", 3, "Mak", 12, 18, 15, 11, 0, 5)]
    [InlineData(2026,  5, 15,  9, "Ben", 6, "Sip", 13, 0, 13, 10, 13, 7)]
    public void Calculate_AnchorDates(
        int year, int month, int day,
        int tzNum, string tzSign,
        int haabDay, string haabMonth,
        int baktun, int katun, int tun, int uinal, int kin,
        int lord)
    {
        var result = Calculator.Calculate(year, month, day);

        Assert.Equal(tzNum, result.Tzolkin.Number);
        Assert.Equal(tzSign, result.Tzolkin.DaySign);
        Assert.Equal(haabDay, result.Haab.Day);
        Assert.Equal(haabMonth, result.Haab.Month);
